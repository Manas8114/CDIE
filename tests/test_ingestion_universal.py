...
from cdie.pipeline.data_merger import DataStoreManager
from cdie.pipeline.data_generator import VARIABLE_NAMES, DATA_DIR

@pytest.fixture
def store_manager(tmp_path):
    master_path = tmp_path / "test_scm_data.csv"
    return DataStoreManager(master_path=master_path)
...
def test_fuzzy_pdf_extraction(monkeypatch):
    ...
    class FakePage:
        def extract_text(self): return dummy_text
    class FakePDF:
    ...
    class MockPdfPlumber:
        def open(self, *args): return FakePDF()
...
def test_data_merging_and_deduplication(store_manager):
    ...
    merged1, warnings1 = store_manager.merge_data(df1)
    assert len(merged1) == 2
    assert store_manager.master_path.exists()
    ...
    merged2, warnings2 = store_manager.merge_data(df2)
...
def test_schema_alignment(store_manager):
    ...
    partial_df = pd.DataFrame({
        "CallDataRecordVolume": [1500],
        "ARPUImpact": [0.5]
    })
    
    merged, warnings = store_manager.merge_data(partial_df)
