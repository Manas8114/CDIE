...
class TestPrescribeEndpoint:
    def test_prescribe_not_loaded(self):
        response = client.post('/prescribe', json={'target': 'NetworkLoad', 'maximize': False, 'limit': 3})
...
class TestExpertCorrectionEndpoint:
    def test_expert_correct_valid(self):
        response = client.post('/expert/correct', json={'from_node': 'A', 'to_node': 'B', 'action': 'add'})
        assert response.status_code == 200
        data = response.json()
        assert data['success'] is True

    def test_expert_correct_invalid_action(self):
        response = client.post('/expert/correct', json={'from_node': 'A', 'to_node': 'B', 'action': 'invalid_action'})
        assert response.status_code == 422
