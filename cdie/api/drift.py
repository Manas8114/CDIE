...
class DriftAnalyzer:
    ...
    def __init__(self, db_path: Path | None = None, history_dir: Path | None = None):
    ...
    def _get_store(self) -> Any:
        ...
        for candidate in self._candidate_paths():
            ...
            try:
                ...
                # Verify readability
                store.execute('PRAGMA schema_version')
    ...
    def _load_json_snapshot(self, snapshot_id: int) -> dict[str, Any] | None:
    ...
    def get_snapshot(self, snapshot_id: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            store = self._get_store()
            row = store.fetch_one(
                'SELECT id, timestamp, edges_json, ate_map_json, n_edges, algorithm, '
                'metadata_json FROM dag_history WHERE id=?',
                (snapshot_id,),
            )
        except Exception:
            return None
        ...
        return {
            ...
            'ate_map': json.loads(row['ate_map_json']),
            'n_edges': row['n_edges'],
            'algorithm': row['algorithm'],
            'metadata': json.loads(row['metadata_json']) if row['metadata_json'] else {},
        }
    ...
    def get_edge_history(self, source: str, target: str) -> list[dict[str, Any]]:
        ...
        for item in timeline:
            ...
            edge_present = any(edge['source'] == source and edge['target'] == target for edge in edges)
            history.append(
                {
