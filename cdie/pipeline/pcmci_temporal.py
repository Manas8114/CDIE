...
def run_temporal_discovery(data: pd.DataFrame, variable_names: list[str] | None = None, max_lag: int = 4) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for _i, src in enumerate(numeric_cols):
            for _j, tgt in enumerate(numeric_cols):
                if src == tgt:
                    continue
                try:
