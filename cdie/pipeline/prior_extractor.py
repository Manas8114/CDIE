...
class PriorExtractor:
        ...
        if self.opea_endpoint:
            try:
                ...
                self.client = OpenAI(
                    ...
                    api_key="opea-placeholder",
                )
                logger.info(
                    f"[PriorExtractor] OPEA TextGen connected at {self.opea_endpoint}"
                )
            except ImportError:
                logger.warning(
                    "[PriorExtractor] OpenAI package not installed. Cannot use LLM Extractor."
                )
            ...
            logger.warning(
                "[PriorExtractor] No LLM endpoint configured. Extraction will fail."
            )
    ...
    def extract_text_from_pdf(self, file_obj: BinaryIO) -> str:
        ...
        try:
            ...
            raise ImportError("pdfplumber is required to extract text from PDF.")
