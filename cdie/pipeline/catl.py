...
def test_faithfulness(data: pd.DataFrame, variable_names: list[str]) -> dict:
    """
    ...
    tooltip = (
        f"Tested {n_tested} variable triples. "
        f"Inconsistency rate: {inconsistency_rate:.1%}. "
    )
    if status == "WARN":
...
def test_causal_sufficiency(data: pd.DataFrame, variable_names: list[str]) -> dict:
    """
...
def test_stationarity(data: pd.DataFrame, variable_names: list[str]) -> dict:
    """
...
def test_positivity(data: pd.DataFrame, variable_names: list[str]) -> dict:
    ...
    if has_adversarial:
        ...
        status = "FAIL"
        tooltip = (
            f"Positivity violation: {', '.join(zero_var_cols)} have zero variance. "
            "Rejecting analysis on these nodes."
        )
    elif low_variance_cols:
        status = "WARN"
        tooltip = (
            f"Low variance in: {', '.join(low_variance_cols)}. "
            "Effect estimates may be unstable."
        )
    else:
        status = "PASS"
        tooltip = (
...
def run_catl(data: pd.DataFrame, variable_names: list[str]) -> dict:
    """Run all CATL tests and return a complete report with adversarial alerts."""
    print("[CATL] Running Causal Assumption Transparency Layer...")

    results = {}

    print("[CATL] Testing positivity...")
    ...
    if pos_status == "ADVERSARIAL_SUSPECTED":
        print(f"[CATL] -> {pos_status} *** SECURITY ALERT ***")
    else:
        print(f"[CATL] -> {pos_status}")

    print("[CATL] Testing faithfulness...")
    results["faithfulness"] = test_faithfulness(data, variable_names)
    print(f"[CATL] -> {results['faithfulness']['status']}")

    print("[CATL] Testing causal sufficiency...")
    results["sufficiency"] = test_causal_sufficiency(data, variable_names)
    print(f"[CATL] -> {results['sufficiency']['status']}")

    print("[CATL] Testing stationarity...")
    results["stationarity"] = test_stationarity(data, variable_names)
    print(f"[CATL] -> {results['stationarity']['status']}")

    print("[CATL] Checking acyclicity...")
    results["acyclicity"] = test_acyclicity()
    print(f"[CATL] -> {results['acyclicity']['status']}")
