...
def _run_subset_test(
    ...
    try:
        ...
        # Avoid division by zero and ensure positive CV
        cv = abs(std_effect / (mean_effect if abs(mean_effect) > 1e-10 else 1e-10))
        passed = cv < 0.25
...
def _dag_to_dot(dag: nx.DiGraph, variable_names: list[str]) -> str:
...
def _run_refutation_fallback(data: pd.DataFrame, treatment: str, outcome: str) -> list[dict[str, Any]]:
    """
    ...
    if len(data) >= 200:
        effects = []
        for _i in range(5):
            subset = data.sample(frac=0.8, random_state=_i)
            model_fit = LinearRegression().fit(subset[[treatment]].values, subset[outcome].values)
            coef = model_fit.coef_[0]
            effects.append(coef)
...
def run_refutation(data: pd.DataFrame, map_dag: nx.DiGraph, variable_names: list[str]) -> dict[str, Any]:
    """
