...
def compute_graph_metrics(discovered_edges: list[tuple[str, str]], true_edges: list[tuple[str, str]], _variable_names: list[str] | None = None) -> dict[str, Any]:
    """
...
def evaluate_sachs() -> dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            data_matrix = data.values if hasattr(data, 'values') else np.array(data)
        except Exception:
...
def evaluate_alarm() -> dict[str, Any]:
    """
...
def evaluate_own_graph(discovered_edges: list[tuple[str, str]], ground_truth_edges: list[tuple[str, str]], variable_names: list[str]) -> dict[str, Any]:
    ...
    metrics['status'] = 'COMPLETE'
    print(
        f'[Benchmark] Own SCM: P={metrics["precision"]}, R={metrics["recall"]}, '
        f'F1={metrics["f1"]}, SHD={metrics["shd"]}'
    )
    return metrics


def run_benchmarks(discovered_edges: list[tuple[str, str]] | None = None, ground_truth_edges: list[tuple[str, str]] | None = None, variable_names: list[str] | None = None) -> dict[str, Any]:
    ...
    if discovered_edges and ground_truth_edges:
        report['own_scm'] = evaluate_own_graph(discovered_edges, ground_truth_edges, variable_names or [])
