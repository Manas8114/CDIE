...
json_path = 'data/safety_map.json'
...
for i, (sid, s) in enumerate(list(scenarios.items())[:5]):
    eff = s.get('effect', {})
    print(f"  {sid}: ATE={eff.get('ate_used', 0):.4f}, point={eff.get('point_estimate', 0):.4f}, CI=[{eff.get('ci_lower', 0):.4f}, {eff.get('ci_upper', 0):.4f}]")
...
# 3. DB schema
print(f"\n=== SQLite tables ===")
conn = sqlite3.connect('data/safety_map.db')
c = conn.cursor()
